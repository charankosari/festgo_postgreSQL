...
const issueTripsPendingCoins = async () => {
  const service_tx = await sequelize.transaction();
  const user_tx = await usersequel.transaction();

  try {
    // 🔍 Step 1: Check if Trips coin issue cron is active
    const cronThing = await CronThing.findOne({
      where: { entity: "trips_coins_issue", active: true },
      transaction: service_tx,
      lock: service_tx.LOCK.UPDATE,
    });
    ...
    // 🔍 Step 2: Find all trips pending coin issues ready to be issued
    const rows = await FestgoCoinToIssue.findAll({
      where: {
        issue: true,
        status: "pending",
        type: "trips_referral", // 👈 Trips type
        issueAt: { [Op.lte]: now },
      },
      transaction: user_tx,
      lock: user_tx.LOCK.UPDATE,
    ...
    if (remaining === 0) {
      await cronThing.update({ active: false }, { transaction: service_tx });
      console.log("✅ Trips: No more coins to issue. Deactivated cron.");
    }
    ...
    await user_tx.rollback();
    await service_tx.rollback();
    console.error("❌ Trips Coin issuing failed:", err);
  }
};
const issueCityFestPendingCoins = async () => {
  const service_tx = await sequelize.transaction();
  const user_tx = await usersequel.transaction();

  try {
    // 🔍 Step 1: Check if CityFest coin issue cron is active
    const cronThing = await CronThing.findOne({
      where: { entity: "cityfest_coins_issue", active: true },
      transaction: service_tx,
      lock: service_tx.LOCK.UPDATE,
    });
    ...
    // 🔍 Step 2: Find all cityfest pending coin issues ready to be issued
    const rows = await FestgoCoinToIssue.findAll({
      where: {
        issue: true,
        status: "pending",
        type: "cityfest_referral", // 👈 CityFest-specific type
        issueAt: { [Op.lte]: now },
      },
      transaction: user_tx,
      lock: user_tx.LOCK.UPDATE,
    ...
    if (remaining === 0) {
      await cronThing.update({ active: false }, { transaction: service_tx });
      console.log("✅ CityFest: No more coins to issue. Deactivated cron.");
    }
    ...
    await user_tx.rollback();
    await service_tx.rollback();
    console.error("❌ CityFest Coin issuing failed:", err);
  }
};
